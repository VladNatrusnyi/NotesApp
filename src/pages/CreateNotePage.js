...
export const CreateNotePage = ({noteText = ''}) => {
    ...
    const [content, setContent] = useState(noteText.trim() ? noteText : '');
    ...
    const writeArticleToDB = async () => {
        setIsLoading(true)
        const db = getDatabase();
        const notesRef = ref(db, 'notes');

        const newNoteIdRef = await push(notesRef);
    ...
    return (
        <View style={styles.wrapper}>
            <View style={styles.toolsPanelWrapper}>
                ...
                <TouchableOpacity disabled={isBtnDisabled} onPress={writeArticleToDB}>
